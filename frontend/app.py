...
# ---- UI definitions ----
guidelines_area = ui.textarea(
    label='Optional guidelines (PEP8, security rules, etc.):',
    placeholder='Follow PEP 8; add docstrings; validate inputs; avoid wildcards',
    value='',
...
def run_review():
    ...
    if not code:
        ui.notify("Please paste some code first.", color='negative')
        return

    ui.notify("Reviewing code... please wait.", color='primary')
    try:
        payload = {"code": code}
        if guidelines_area.value.strip():
            payload["guidelines"] = guidelines_area.value.strip()
        res = requests.post(BACKEND_URL, data=payload)
        data = res.json()
        ...
        if feedbacks:
            ...
            for fb in feedbacks:
                ...
                if effort:
                    ai_md += f"Effort: {effort}\n\n"
                if fb.get('doc_update'):
                    ai_md += f"Doc update: {fb['doc_update']}\n\n"
